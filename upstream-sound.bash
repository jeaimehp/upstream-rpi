#!/bin/bash
RECORDTIME=$1
HWDEVICE=`echo "$(arecord -l |grep card | sed 's/.*card //' | cut -d " " -f 1|cut -c 1),$(arecord -l |grep card | sed 's/.*device //' | cut -d " " -f 1|cut -c 1)"`
if [ -z "$RECORDTIME" ]
then
	...
	/usr/bin/arecord --duration=$RECORDTIME --channels=2 --format=S16_LE --rate 44100 --device=hw:$HWDEVICE /home/pi/upstream/sound/`(date +%s)`.wav
