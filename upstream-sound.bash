...
then
	...
	/usr/bin/arecord --duration=10 --format=cd --file-type=wav --device=hw:$HWDEVICE /home/pi/upstream/sound/`(date +%s)`.wav


	echo "Recorded sound @ $(date)"
else
	/usr/bin/arecord --duration=$RECORDTIME --channels=2 --format=cd --rate 44100 --device=hw:$HWDEVICE /home/pi/upstream/sound/`(date +%s)`.wav
